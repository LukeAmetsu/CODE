...
function draw2dSpliceDiagram(targetSvg = null, viewMode = "all") {
  ...
  const drawLine = (parent, x1, y1, x2, y2, type = "solid") => {
    ...
    l.setAttribute("x1", x1 * S);
    l.setAttribute("y1", y1 * S);
    l.setAttribute("x2", x2 * S);
    l.setAttribute("y2", y2 * S);
  ...
  const drawBoltFace = (parent, cx, cy, dia) => {
    ...
    c.setAttribute("cx", cx * S);
    c.setAttribute("cy", cy * S);
    c.setAttribute("r", dia * S * 0.3);
  ...
  const drawBoltSide = (parent, x, y, len, dia) => {
    ...
    rect.setAttribute("x", (x - dia / 2) * S);
    rect.setAttribute("y", y * S);
    rect.setAttribute("width", dia * S);
    rect.setAttribute("height", len * S);
    rect.setAttribute("fill", "#ddd");
    rect.setAttribute("stroke", "#000");
    parent.appendChild(rect);
    // Head
    const head = document.createElementNS(ns, "rect");
    head.setAttribute("x", (x - dia * 0.8) * S);
    head.setAttribute("y", (y - headH) * S);
    head.setAttribute("width", dia * 1.6 * S);
    head.setAttribute("height", headH * S);
    head.setAttribute("fill", "#fff");
    head.setAttribute("stroke", "#000");
    parent.appendChild(head);
    // Nut
    const nut = document.createElementNS(ns, "rect");
    nut.setAttribute("x", (x - dia * 0.8) * S);
    nut.setAttribute("y", (y + len) * S);
    nut.setAttribute("width", dia * 1.6 * S);
    nut.setAttribute("height", nutH * S);
    nut.setAttribute("fill", "#fff");
  ...
  const drawDim = (parent, x1, y1, x2, y2, offset, text, vertical = false) => {
    ...
    if (vertical) {
      ...
      l1.setAttribute("x1", tx1);
      l1.setAttribute("y1", ty1);
      l1.setAttribute("x2", lineX);
      l1.setAttribute("y2", ty1);
      l1.setAttribute("stroke", "#000");
      l1.setAttribute("stroke-width", "0.5");
      g.appendChild(l1);

      const l2 = document.createElementNS(ns, "line");
      l2.setAttribute("x1", tx2);
      l2.setAttribute("y1", ty2);
      l2.setAttribute("x2", lineX);
      l2.setAttribute("y2", ty2);
      l2.setAttribute("stroke", "#000");
      l2.setAttribute("stroke-width", "0.5");
      g.appendChild(l2);

      const main = document.createElementNS(ns, "line");
      main.setAttribute("x1", lineX);
      main.setAttribute("y1", ty1);
      main.setAttribute("x2", lineX);
      main.setAttribute("y2", ty2);
      main.setAttribute("stroke", "#000");
      main.setAttribute("stroke-width", "0.5");
      main.setAttribute("marker-start", "url(#arrow)");
      main.setAttribute("marker-end", "url(#arrow)");
      g.appendChild(main);

      const txt = document.createElementNS(ns, "text");
      txt.setAttribute("x", lineX - 5);
      txt.setAttribute("y", (ty1 + ty2) / 2);
      ...
      l1.setAttribute("x1", tx1);
      l1.setAttribute("y1", ty1);
      l1.setAttribute("x2", tx1);
      l1.setAttribute("y2", lineY);
      l1.setAttribute("stroke", "#000");
      l1.setAttribute("stroke-width", "0.5");
      g.appendChild(l1);
      ...
      l2.setAttribute("x1", tx2);
      l2.setAttribute("y1", ty2);
      l2.setAttribute("x2", tx2);
      l2.setAttribute("y2", lineY);
      l2.setAttribute("stroke", "#000");
      l2.setAttribute("stroke-width", "0.5");
      g.appendChild(l2);

      const main = document.createElementNS(ns, "line");
      main.setAttribute("x1", tx1);
      main.setAttribute("y1", lineY);
      main.setAttribute("x2", tx2);
      main.setAttribute("y2", lineY);
      main.setAttribute("stroke", "#000");
      main.setAttribute("stroke-width", "0.5");
      main.setAttribute("marker-start", "url(#arrow)");
      main.setAttribute("marker-end", "url(#arrow)");
      ...
      const txt = document.createElementNS(ns, "text");
      txt.setAttribute("x", (tx1 + tx2) / 2);
      txt.setAttribute("y", lineY - 2.5);
  ...
  const drawLegendBox = (parent, x, y, lines, opts = {}) => {
    ...
    // lines
    lines.forEach((ln, i) => {
      drawText(parent, x + pad, y + pad + i * lineH, ln, {
        fontSize: fontSizePx,
      });
    });
  ...
  if (viewMode === "all" || viewMode === "elevation") {
    ...
    drawText(elevG, 0, -d / 2 - 3.0, "ELEVATION", {
      anchor: "middle",
      bold: true,
      fontSize: 12,
    });
    ...
    if (num_fp > 0) {
      ...
      [-1, 1].forEach((side) => {
        for (let c = 0; c < Nc_f; c++) {
          ...
          const totalThick =
            t_fp + tf + (num_fp == 2 ? safeFloat(inputs.t_fp_inner, 0) : 0);
          drawBoltSide(
            elevG,
            bx,
            d / 2 - tf - (num_fp == 2 ? safeFloat(inputs.t_fp_inner, 0) : 0),
            totalThick,
            D_fp,
          );
    ...
    if (num_wp > 0) {
      elevLegendLines.push(
        `${num_wp}x PL ${t_wp.toFixed(3)} x ${H_wp} x ${L_wp}`,
      );
      const Nc_w = parseInt(inputs.Nc_wp) || 0;
      const Nr_w = parseInt(inputs.Nr_wp) || 0;
      if (Nc_w && Nr_w)
        elevLegendLines.push(`Web Bolts: ${Nc_w * Nr_w * 2} @ Ø${D_wp}"`);
    }
    if (num_fp > 0) {
      elevLegendLines.push(
        `Outer FLG: PL ${t_fp.toFixed(3)} x ${H_fp} x ${L_fp}`,
      );
      if (num_fp === 2) {
        elevLegendLines.push(
          `Inner FLG: 2x PL ${safeFloat(inputs.t_fp_inner, 0).toFixed(3)} x ${safeFloat(inputs.H_fp_inner, 0)} x ${safeFloat(inputs.L_fp_inner, 0)}`,
        );
      }
      const Nc_f = parseInt(inputs.Nc_fp) || 0;
      const Nr_f = parseInt(inputs.Nr_fp) || 0;
      if (Nc_f && Nr_f)
        elevLegendLines.push(`Flange Bolts: ${Nc_f * Nr_f * 4} @ Ø${D_fp}"`);
    }
    if (elevLegendLines.length) {
      // Place legend in the Gutter (far left relative to group center)
      drawLegendBox(
        elevG,
        -beamLen / 2 - legendWidth,
        -d / 2 - 2.6,
        elevLegendLines,
        { w: 8.0, fontSize: 6.5 },
      ); // Larger font for legend vs dims
    }
  }
  ...
  if (viewMode === "all" || viewMode === "section") {
    const sectX =
      viewMode === "all"
        ? legendWidth + beamLen + 4 + bf / 2
        : viewMode === "section"
          ? legendWidth / 2 + beamLen / 4
          : 0;
    ...
    const finalSectX =
      viewMode === "all" ? legendWidth + beamLen + 4 + bf / 2 : 0;
    const sectG = createGroup(svg, finalSectX, d / 2 + 7);
    drawText(sectG, 0, -d / 2 - 3.0, "SECTION", {
      anchor: "middle",
      bold: true,
      fontSize: 12,
    });
    ...
    if (num_wp > 0) {
      ...
      boltRect.setAttribute("x", (-boltL / 2) * S);
      boltRect.setAttribute("y", (-D_wp / 2) * S);
      boltRect.setAttribute("width", boltL * S);
      boltRect.setAttribute("height", D_wp * S);
      boltRect.setAttribute("fill", "#ddd");
      boltRect.setAttribute("stroke", "#000");
  ...
  if (viewMode === "all" || viewMode === "plan") {
    ...
    drawText(planG, 0, -bf / 2 - 2.5, "PLAN (TOP FLANGE)", {
      anchor: "middle",
      bold: true,
      fontSize: 12,
    });
    ...
    if (num_fp > 0) {
      ...
      boltText.setAttribute("x", 0);
      boltText.setAttribute("y", (H_fp / 2 + 3) * S);
      boltText.setAttribute("text-anchor", "middle");
      boltText.setAttribute("font-size", "10");
...
function draw3dSpliceDiagram() {
  ...
  if (dimensionElements) {
    if (dimensionElements.meshes)
      dimensionElements.meshes.forEach((m) => m.dispose());
    if (dimensionElements.labels)
      dimensionElements.labels.forEach((l) => l.dispose());
  }
  ...
  if (!inputs.member_d || typeof inputs.member_d !== "number")
    inputs.member_d = 0;
  ...
  // --- 3D Dimension Helper ---
  const create3DDimension = (
    p1,
    p2,
    text,
    offset,
    planeNormal = new BABYLON.Vector3(0, 1, 0),
  ) => {
    ...
    const mainLine = BABYLON.MeshBuilder.CreateLines(
      "dimLine",
      { points: [p1_ext, p2_ext] },
      bjsScene,
    );
    ...
    const arrow2 = BABYLON.MeshBuilder.CreateCylinder(
      "a2",
      {
        diameterTop: 0,
        diameterBottom: arrowSize / 2,
    ...
    if (dimensionElements && dimensionElements.meshes) {
      ...
      const vis =
        typeof areDimensionsVisible !== "undefined"
          ? areDimensionsVisible
          : true;
      [ext1, ext2, mainLine, arrow1, arrow2, plane].forEach(
        (m) => (m.isVisible = vis),
      );
    }
  };
  ...
  if (inputs.num_web_plates > 0) {
    ...
    const wp1 = BABYLON.MeshBuilder.CreateBox(
      "wp1",
      { width: t, height: H, depth: L },
      bjsScene,
    );
  ...
  const getBoltMesh = (dia, len) => {
    ...
    const head = BABYLON.MeshBuilder.CreateCylinder(
      "h",
      { diameter: dia * 1.6, height: dia * 0.6, tessellation: 6 },
      bjsScene,
    );
    head.position.y = len / 2 + dia * 0.3;
    const shank = BABYLON.MeshBuilder.CreateCylinder(
      "s",
      { diameter: dia, height: len },
      bjsScene,
    );
    ...
    const master = BABYLON.Mesh.MergeMeshes(
      [head, shank, nut],
      true,
      true,
      undefined,
  ...
  if (inputs.num_flange_plates > 0 && inputs.Nc_fp > 0) {
    ...
    const centerTop =
      inputs.member_d / 2 + (inputs.t_fp - inputs.member_tf - t_in) / 2;
    const centerBot =
      -inputs.member_d / 2 - (inputs.t_fp - inputs.member_tf - t_in) / 2;
...
const spliceCalculator = (() => {
  ...
  function checkBoltShear({
    grade,
    threadsIncl,
    db,
    numPlanes = 1,
    fastenerPatternLength = 0,
    jurisdiction,
  }) {
    const { Fnv, wasReduced } = AISC_SPEC.getFnv(
      grade,
      threadsIncl,
      fastenerPatternLength,
    );
    const Ab = (PI * db ** 2) / 4.0;
    const factors = getDesignFactors(jurisdiction, 0.75, 2.0);
    return {
      Rn: Fnv * Ab * numPlanes,
      ...factors,
      Fnv,
  ...
  function checkBoltBearing({
    ...
    if (Lc < 0 || t_ply <= 0)
      return { Rn: 0, ...factors, Lc: 0, Rn_tearout: 0, Rn_bearing: 0 };
  ...
  function computeShearLagFactorU({
    plate_width,
    gage,
    num_fastener_rows,
    conn_length,
    ...
    const x_bar =
      total_area > 0
        ? (area_flange * (flange_width / 2.0) -
            area_stem * (stem_height / 2.0)) /
          total_area
        : 0;
    const U_case2 = 1.0 - Math.abs(x_bar) / conn_length;
    const U_case7 = plate_width >= (2 / 3) * gage ? 0.9 : 0.85;

    return {
      U: Math.min(1.0, Math.max(U_case2, U_case7)),
      U_case2,
      U_case7,
      x_bar: Math.abs(x_bar),
    };
  }

  function checkGrossSectionYielding({ Ag, Fy, jurisdiction }) {
    const factors = getDesignFactors(jurisdiction, 0.9, 1.67);
    return { Rn: Fy * Ag, ...factors, Ag, Fy };
  }

  function checkFlangeNetSection({
    bf,
    tf,
    Fu,
    num_bolts_in_cs,
  ...
  function checkBlockShear({ Agv, Anv, Ant, Fu, Fy, Ubs = 1.0, jurisdiction }) {
    ...
    const Rn = Math.min(
      shear_rupture_term + tension_rupture_term,
      shear_yield_term + tension_rupture_term,
    );
    const factors = getDesignFactors(jurisdiction, 0.75, 2.0);
  ...
  function checkPlateCompression({
    Ag,
    Fy,
    t,
    unbraced_length,
  ...
  function checkBeamFlexuralRupture(
    Sx,
    Fy,
    Fu,
    bf,
    tf,
    num_bolts_in_flange_cs,
    hole_dia_net_area,
    jurisdiction,
  ) {
    const Afg = bf * tf;
    ...
    return {
      Rn: Mn_rupture_kip_in,
      ...factors,
      Mn_rupture: Mn_rupture_kip_in,
      Afg,
      Afn,
      Yt,
      Sx,
  ...
  function checkBoltShearTensionInteraction(
    Tu,
    Vu,
    Fnv,
    grade,
    ...
    if (design_method === "LRFD") {
      ...
      F_nt_prime = 1.3 * Fnt - (((2.0 / 0.75) * Fnt) / Fnv) * fv;
    }
  ...
  function calculateWebSpliceEccentricity(
    V_load,
    H_load,
    gap,
    Nc,
  ...
  function checkPryingAction(params) {
    const { t_plate, Fy_plate, b, a, p, d_bolt, d_hole, B_bolt, jurisdiction } =
      params;
  ...
  function getGeometryChecks({
    ...
    return {
      edge_dist_long: {
        actual: le_long,
        min: min_le,
        pass: le_long >= min_le - tolerance,
      },
      edge_dist_tran: {
        actual: le_tran,
        min: min_le,
        pass: le_tran >= min_le - tolerance,
      },
      spacing_col: {
        actual: s_col,
        min: min_s,
        pass: s_col >= min_s - tolerance,
      },
      spacing_row: {
        actual: s_row,
        min: min_s,
        pass: s_row >= min_s - tolerance,
      },
      spacing_gage: {
        actual: gage,
        min: min_s,
        pass: !gage || gage >= min_s - tolerance,
      },
      max_spacing_col: {
        actual: s_col,
        max: max_s,
        pass: s_col <= max_s + tolerance,
      },
      max_spacing_row: {
        actual: s_row,
        max: max_s,
        pass: s_row <= max_s + tolerance,
      },
    };
  }

  function calculateBoltGroupGeometry({
    L_plate,
    H_plate,
    Nc,
    Nr,
  ...
  function performPlateChecks(plateName, inputs, config) {
    ...
    plateChecks[`${plateName} GSY`] = {
      demand,
      check: checkGrossSectionYielding({
        Ag,
        Fy,
    ...
    plateChecks[`${plateName} Compression`] = {
      demand: demand_comp,
      check: checkPlateCompression({
        Ag,
        Fy,
        t: t_p,
        unbraced_length: S_col,
        jurisdiction: inputs.jurisdiction,
      }),
    };
    ...
    const bearing_edge = checkBoltBearing({
      db: D_bolt,
      t_ply: t_p,
      Fu_ply: Fu,
      le: le_long,
      s: S_col,
      isEdgeBolt: true,
      deformationIsConsideration: inputs.deformation_is_consideration,
      hole_dia: hole_for_bearing,
      jurisdiction: inputs.jurisdiction,
    });
    const bearing_int = checkBoltBearing({
      db: D_bolt,
      t_ply: t_p,
      Fu_ply: Fu,
      le: le_long,
    ...
    const total_bearing =
      bearing_edge.Rn * num_edge_bolts + bearing_int.Rn * num_int_bolts;
    plateChecks[`${plateName} Bolt Bearing`] = {
      demand,
      check: {
        Rn: total_bearing,
        ...getDesignFactors(inputs.jurisdiction, 0.75, 2.0),
      },
      details: {
        edge: bearing_edge,
        int: bearing_int,
  ...
  function performFlangeChecks(inputs, demands) {
    ...
    const {
      total_flange_demand_tension,
      demand_fp_outer,
      demand_fp_inner,
      demand_fp_outer_comp,
      demand_fp_inner_comp,
    } = demands;
    const checks = {};
    const geomChecks = {};
    ...
    const {
      le_long: le_long_fp,
      le_tran: le_tran_fp,
      edge_dist_gap: edge_dist_gap_fp,
      bolt_pattern_height: bolt_pattern_height_fp,
    } = calculateBoltGroupGeometry({
      L_plate: inputs.L_fp,
      H_plate: inputs.H_fp,
      Nc: inputs.Nc_fp,
      Nr: inputs.Nr_fp,
    ...
    const fastenerPatternLength_flange =
      inputs.Nc_fp > 1 ? (inputs.Nc_fp - 1) * inputs.S1_col_spacing_fp : 0;
    ...
    checks["Flange Bolt Shear"] = {
      ...
      check: {
        ...single_bolt_shear_fp_check,
        Rn: single_bolt_shear_fp_check.Rn * num_flange_bolts_per_side,
      },
      details: {
    ...
    Object.assign(
      checks,
      performPlateChecks("Outer Plate", inputs, {
        demand: demand_fp_outer,
        demand_comp: demand_fp_outer_comp,
        H_p: inputs.H_fp,
        t_p: inputs.t_fp,
        L_p: inputs.L_fp,
        Fy: inputs.flange_plate_Fy,
        Fu: inputs.flange_plate_Fu,
        Nc: inputs.Nc_fp,
        Nr: inputs.Nr_fp,
        S_col: inputs.S1_col_spacing_fp,
        S_row: inputs.S2_row_spacing_fp,
        S_end: inputs.S3_end_dist_fp,
        gage: inputs.g_gage_fp,
        D_bolt: D_fp_num,
        hole_for_net_area: hole_for_net_area_fp,
        hole_for_bearing: hole_for_bearing_fp,
      }),
    );
    if (inputs.num_flange_plates === 2) {
      Object.assign(
        checks,
        performPlateChecks("Inner Plate", inputs, {
          demand: demand_fp_inner,
          demand_comp: demand_fp_inner_comp,
          H_p: inputs.H_fp_inner,
          t_p: inputs.t_fp_inner,
          L_p: inputs.L_fp_inner,
          Fy: inputs.flange_plate_Fy_inner,
          Fu: inputs.flange_plate_Fu_inner,
          Nc: inputs.Nc_fp,
          Nr: inputs.Nr_fp,
          S_col: inputs.S1_col_spacing_fp,
          S_row: inputs.S2_row_spacing_fp,
    ...
    const L_gv_beam_f =
      inputs.S3_end_dist_fp + (inputs.Nc_fp - 1) * inputs.S1_col_spacing_fp;
    const L_nv_beam_f =
      L_gv_beam_f - (inputs.Nc_fp - 0.5) * hole_for_net_area_fp;
    const Ant_beam_f =
    ...
    checks["Beam Flange Block Shear"] = {
      ...
      details: {
        t_p: inputs.member_tf,
        hole_dia: hole_for_net_area_fp,
        Agv: L_gv_beam_f * inputs.member_tf,
        Anv: L_nv_beam_f * inputs.member_tf,
        Agt: Agt_beam_f,
        Ant: Ant_beam_f,
    ...
    Object.assign(
      checks,
      performBeamConnectionChecks("Flange", inputs, {
        demand: total_flange_demand_tension,
        t_beam: inputs.member_tf,
        Fu_beam: inputs.member_Fu,
        Fy_beam: inputs.member_Fy,
        Nc: inputs.Nc_fp,
        Nr: inputs.Nr_fp,
        S_col: inputs.S1_col_spacing_fp,
        S_row: inputs.S2_row_spacing_fp,
    ...
    const B_per_bolt =
      num_flange_bolts_per_side > 0
        ? total_flange_demand_tension / num_flange_bolts_per_side
        : 0;
    ...
    if (B_per_bolt > 0) {
      if (inputs.num_flange_plates === 2) {
        ...
        const prying_outer = checkPryingAction({
          t_plate: inputs.t_fp,
        ...
        const prying_inner = checkPryingAction({
          t_plate: inputs.t_fp_inner,
          Fy_plate: inputs.flange_plate_Fy_inner,
          b: b_pry_inner,
          a: a_pry_inner,
          p: inputs.S1_col_spacing_fp,
          d_bolt: D_fp_num,
        ...
        const prying_result = checkPryingAction({
          t_plate: inputs.t_fp,
          Fy_plate: inputs.flange_plate_Fy,
          b: b_pry,
          a: a_pry,
      ...
      checks["Flange Bolt Tension with Prying"] = {
        demand: T_req,
        check: checkBoltTension(
          inputs.bolt_grade_fp,
          D_fp_num,
          inputs.jurisdiction,
        ),
    ...
    checks["Plate Thickness for Prying"] = {
      demand: inputs.t_fp,
      check: { Rn: outer_plate_tc, phi: 1.0, omega: 1.0 },
      details: prying_details,
    };
    const t_thinner_flange = min(
      inputs.member_tf,
      inputs.t_fp,
      inputs.num_flange_plates === 2 ? inputs.t_fp_inner : Infinity,
    );
    ...
    geomChecks["Flange Bolts"].edge_dist_gap = {
      actual: edge_dist_gap_fp,
      min: min_le_fp,
      pass: edge_dist_gap_fp >= min_le_fp - tolerance,
    };
    return { checks, geomChecks, inputs };
  }

  function boltCoefficient(
    boltRow,
    boltColumn,
    rowSpacing,
    columnSpacing,
    ...
    for (let i = 0; i < Rv; i++) {
      for (let k = 0; k < Rh; k++) {
        let y1 = i * Sv - ((Rv - 1) * Sv) / 2;
        let x1 = k * Sh - ((Rh - 1) * Sh) / 2;
    ...
    while (!stp) {
      ...
      const ro = Ec + xRo;
      p = Math.abs(ro) > epsilon ? m / ro : 0;
      const uFy = p - ry;
      const uFx = -rx;
      const uF = Math.sqrt(uFy ** 2 + uFx ** 2);
  ...
  function performWebChecks(inputs, demands) {
    ...
    const {
      le_long: le_long_wp,
      le_tran: le_tran_wp,
      edge_dist_gap: edge_dist_gap_wp,
    } = calculateBoltGroupGeometry({
      L_plate: inputs.L_wp,
      H_plate: inputs.H_wp,
      Nc: inputs.Nc_wp,
    ...
    if (inputs.connection_type === "slip-critical") {
      ...
      const total_slip_capacity =
        single_bolt_slip_check.Rn * num_web_bolts_per_side;
      checks["Web Bolt Slip"] = {
        demand: Math.sqrt(V_load ** 2 + Hw ** 2),
        check: { ...single_bolt_slip_check, Rn: total_slip_capacity },
        details: { num_bolts: num_web_bolts_per_side },
      };
    }
    const fastenerPatternLength_web =
      inputs.Nr_wp > 1 ? (inputs.Nr_wp - 1) * inputs.S5_row_spacing_wp : 0;
    ...
    const eccentricity = calculateWebSpliceEccentricity(
      1,
      0,
      inputs.gap,
      inputs.Nc_wp,
      inputs.Nr_wp,
      inputs.S4_col_spacing_wp,
      inputs.S5_row_spacing_wp,
      inputs.S6_end_dist_wp,
    ).eccentricity;
    ...
    const factors_bolt_group = getDesignFactors(inputs.jurisdiction, 0.75, 2.0);
    ...
    if (Tu_per_bolt > 0) {
      checks["Web Bolt Shear/Tension Interaction"] = {
        demand: Tu_per_bolt,
        check: checkBoltShearTensionInteraction(
          Tu_per_bolt,
          Vu_per_bolt,
          single_web_bolt_shear_check.Fnv,
          inputs.bolt_grade_wp,
          D_wp_num,
    ...
    const Anv_wp =
      (inputs.H_wp - inputs.Nr_wp * hole_for_net_area_wp) * total_t_wp;
    checks["Web Plate Net Shear Rupture"] = {
      demand: V_load,
      check: checkShearRupture(
        Anv_wp,
        inputs.web_plate_Fu,
        inputs.jurisdiction,
      ),
    ...
    if (Mu_resisted_by_web > 0) {
      ...
      const flex_yield_factors = getDesignFactors(
        inputs.jurisdiction,
        0.9,
        1.67,
      );
      ...
      const An_wp_flexure =
        (inputs.H_wp - inputs.Nr_wp * hole_for_net_area_wp) * total_t_wp;
      const Mn_rupture_wp =
        (inputs.web_plate_Fu * An_wp_flexure * Sx_wp) /
        (inputs.H_wp * total_t_wp);
      checks["Web Plate Flexural Rupture"] = {
        demand: Mu_resisted_by_web,
        check: {
          Rn: Mn_rupture_wp,
          ...getDesignFactors(inputs.jurisdiction, 0.75, 2.0),
    ...
    const L_gv_single_path =
      inputs.S6_end_dist_wp + (inputs.Nc_wp - 1) * inputs.S4_col_spacing_wp;
    ...
    const bearing_wp_plate_edge = checkBoltBearing({
      db: D_wp_num,
      t_ply: total_t_wp,
      Fu_ply: inputs.web_plate_Fu,
      le: le_long_wp,
      s: inputs.S4_col_spacing_wp,
      isEdgeBolt: true,
      deformationIsConsideration: inputs.deformation_is_consideration,
    ...
    const total_bearing_capacity_plate =
      bearing_wp_plate_edge.Rn * num_edge_bolts_web +
      bearing_wp_plate_int.Rn * num_int_bolts_web;
    checks["Web Plate Bolt Bearing"] = {
      demand: V_load,
      check: { ...bearing_wp_plate_edge, Rn: total_bearing_capacity_plate },
      details: {
        edge: bearing_wp_plate_edge,
        int: bearing_wp_plate_int,
        num_edge: num_edge_bolts_web,
        num_int: num_int_bolts_web,
      },
    };
    Object.assign(
      checks,
      performBeamConnectionChecks("Web", inputs, {
        demand: V_load,
        t_beam: inputs.member_tw,
        Fu_beam: inputs.member_Fu,
        Fy_beam: inputs.member_Fy,
        Nc: inputs.Nc_wp,
        Nr: inputs.Nr_wp,
        S_col: inputs.S4_col_spacing_wp,
        S_row: inputs.S5_row_spacing_wp,
        S_end: inputs.S6_end_dist_wp,
        D_bolt: D_wp_num,
        hole_for_bearing: hole_for_bearing_wp,
      }),
    );
    const t_thinner_web = min(
      inputs.member_tw,
      inputs.t_wp * inputs.num_web_plates,
    );
    ...
    geomChecks["Web Bolts"].edge_dist_gap = {
      actual: edge_dist_gap_wp,
      min: min_le_wp,
      pass: edge_dist_gap_wp >= min_le_wp - tolerance,
    };
    return { checks, geomChecks, inputs };
  }
  ...
  function performMemberChecks(inputs, demands, calculated_holes) {
    ...
    const checks = {};
    const effective_hole_for_net_area_fp =
      hole_for_net_area_fp ||
      AISC_SPEC.getNominalHoleDiameter(inputs.D_fp) + 1.0 / 16.0;
    const effective_hole_for_net_area_wp =
      hole_for_net_area_wp ||
      AISC_SPEC.getNominalHoleDiameter(inputs.D_wp) + 1.0 / 16.0;
    const Mn_yield = inputs.member_Fy * inputs.member_Zx;
    const flex_yield_factors = getDesignFactors(inputs.jurisdiction, 0.9, 1.67);
    checks["Beam Flexural Yielding"] = {
      demand: M_load * 12,
      check: {
        Rn: Mn_yield,
        phi: flex_yield_factors.phi,
        omega: flex_yield_factors.omega,
        Fy: inputs.member_Fy,
        Zx: inputs.member_Zx,
      },
    };
    const Agv_beam_web =
      (inputs.member_d - 2 * inputs.member_tf) * inputs.member_tw;
    ...
    checks["Beam Flexural Rupture"] = {
      demand: M_load * 12,
      check: checkBeamFlexuralRupture(
        inputs.member_Sx,
        inputs.member_Fy,
        inputs.member_Fu,
        inputs.member_bf,
        inputs.member_tf,
        num_bolts_in_flange_cs,
        effective_hole_for_net_area_fp,
        inputs.jurisdiction,
      ),
    };
    const Anv_beam_web =
      (inputs.member_d -
        2 * inputs.member_tf -
        inputs.Nr_wp * effective_hole_for_net_area_wp) *
      inputs.member_tw;
    checks["Beam Web Shear Rupture"] = {
      ...
      details: {
        ...
        tf: inputs.member_tf,
        Nr_wp: inputs.Nr_wp,
        hole_dia: effective_hole_for_net_area_wp,
        tw: inputs.member_tw,
      },
    };
    ...
    if (Axial_load !== 0) {
      ...
      const moment_capacity = Math.min(
        flexural_yielding.Rn,
        flexural_rupture.Rn,
      );
      const moment_factors = getDesignFactors(inputs.jurisdiction, 0.9, 1.67);
      checks["Spliced Member Moment Capacity"] = {
        demand: M_load * 12,
        check: {
          Rn: moment_capacity,
          phi: moment_factors.phi,
          omega: moment_factors.omega,
        },
        details: { yielding: flexural_yielding, rupture: flexural_rupture },
      };
    }
    if (Axial_load > 0) {
      const A_gross_approx =
        2 * inputs.member_bf * inputs.member_tf +
        (inputs.member_d - 2 * inputs.member_tf) * inputs.member_tw;
      const A_holes_flange =
        2 * inputs.Nr_fp * effective_hole_for_net_area_fp * inputs.member_tf;
      const A_holes_web =
        inputs.Nr_wp * effective_hole_for_net_area_wp * inputs.member_tw;
      const An = A_gross_approx - 2 * A_holes_flange - A_holes_web;
      const U_case7 =
        inputs.member_bf >= (2 / 3) * inputs.member_d ? 0.9 : 0.85;
      const conn_length_flange = (inputs.Nc_fp - 1) * inputs.S1_col_spacing_fp;
      const { U: U_case2, x_bar } = computeShearLagFactorU({
        plate_width: inputs.member_bf,
        gage: inputs.g_gage_fp,
        num_fastener_rows: inputs.Nr_fp,
        conn_length: conn_length_flange,
        t_p: inputs.member_tf,
        d_bolt: inputs.D_fp,
      });
      ...
      check.phi = getDesignFactors(inputs.jurisdiction, 0.75, 2.0).phi;
      check.omega = getDesignFactors(inputs.jurisdiction, 0.75, 2.0).omega;
  ...
  function performChecks(inputs, M_load, V_load) {
    ...
    const total_flange_demand_tension =
      flange_force_from_moment + axial_per_flange;
    const total_flange_demand_compression =
      flange_force_from_moment - axial_per_flange;
    const demand_fp_outer =
      inputs.num_flange_plates === 2
        ? total_flange_demand_tension * 0.5
        : total_flange_demand_tension;
    const demand_fp_inner =
      inputs.num_flange_plates === 2 ? total_flange_demand_tension * 0.5 : 0;
    const demand_fp_outer_comp =
      inputs.num_flange_plates === 2
        ? Math.abs(total_flange_demand_compression) * 0.5
        : Math.abs(total_flange_demand_compression);
    const demand_fp_inner_comp =
      inputs.num_flange_plates === 2
        ? Math.abs(total_flange_demand_compression) * 0.5
        : 0;

    const flangeResults = performFlangeChecks(inputs, {
      total_flange_demand_tension,
      total_flange_demand_compression,
      demand_fp_outer,
      demand_fp_inner,
      demand_fp_outer_comp,
      demand_fp_inner_comp,
    });
    const flange_capacity_for_moment =
      flangeResults.checks["Flange Bolt Shear"]?.check?.Rn ?? 0;
    const Mu_flange_splice_capacity =
      flange_capacity_for_moment * moment_arm_flange;
    const Mu_resisted_by_web = Math.max(
      0,
      Math.abs(M_load * 12) - Mu_flange_splice_capacity,
    );
    const Hw = inputs.H_wp > 0 ? Mu_resisted_by_web / (inputs.H_wp * 0.75) : 0;
    const webResults = performWebChecks(inputs, {
      V_load,
      Hw,
      Mu_resisted_by_web,
    });
    const calculated_holes = {
      hole_for_net_area_fp:
        flangeResults.checks["Beam Flange Tensile Rupture"]?.check
          ?.hole_dia_net_area,
      hole_for_net_area_wp:
        webResults.checks["Web Plate Net Shear Rupture"]?.details?.hole_dia,
    };
    const memberResults = performMemberChecks(
      inputs,
      { M_load, V_load, Axial_load: inputs.Axial_load },
      calculated_holes,
    );

    return {
      checks: {
        ...flangeResults.checks,
        ...webResults.checks,
        ...memberResults.checks,
      },
  ...
  function runSingleCheck(inputs) {
    ...
    if (inputs.develop_capacity_check) {
      ...
      if (Zx > 0) {
        ...
        const omega_b = 1.67;
        M_load =
          (inputs.design_method === "LRFD"
            ? phi_b * Mn_kipin
            : Mn_kipin / omega_b) / 12.0;
      }
      const Aw = (inputs.member_d - 2 * inputs.member_tf) * inputs.member_tw;
      if (Aw > 0) {
        const Vn_kips = 0.6 * inputs.member_Fy * Aw;
        const phi_v_yield = 1.0;
        const omega_v_yield = 1.5;
        V_load =
          inputs.design_method === "LRFD"
            ? phi_v_yield * Vn_kips
            : Vn_kips / omega_v_yield;
      }
    }
  ...
  function optimizeFlangeBolts(inputs, optimizationLog) {
    ...
    const flangeBoltDiameters = inputs.optimize_diameter_check
      ? AISC_SPEC.standardBoltDiameters
      : [inputs.D_fp];
    ...
    for (const d_fp of flangeBoltDiameters) {
      for (
        let total_bolts = 2;
        total_bolts <= MAX_TOTAL_BOLTS_PER_SIDE;
        total_bolts += 2
      ) {
        ...
        for (let nc_fp = 1; nc_fp <= MAX_ROWS_OR_COLS; nc_fp++) {
          if (total_bolts % (2 * nc_fp) === 0) {
            ...
            const failingStrengthCheck = Object.entries(results.checks)
              .filter(
                ([key]) =>
                  (key.includes("Flange Bolt") || key.includes("Plate")) &&
                  !key.startsWith("Beam"),
              )
              .find(([key, data]) => {
                const capacity =
                  inputs.design_method === "LRFD"
                    ? data.check.Rn * data.check.phi
                    : data.check.Rn / data.check.omega;
                const ratio =
                  capacity > 0 ? Math.abs(data.demand) / capacity : Infinity;
                return ratio > 1.0;
              });
            const failingGeomCheck = Object.entries(
              results.geomChecks["Flange Bolts"] || {},
            ...
            if (!failingStrengthCheck && !failingGeomCheck) {
              ...
              if (failingStrengthCheck)
                lastFailureReason = `Strength check failed: ${failingStrengthCheck[0]}`;
              else if (failingGeomCheck)
                lastFailureReason = `Geometry check failed: ${failingGeomCheck[0]}`;
            }
          }
        }
      }
    ...
    if (bestSolution) {
      ...
      optimizationLog.push(
        `Flange splice optimized to ${Nc_fp} column(s) and ${Nr_fp} row(s) of ${D_fp}" bolts (${bestSolution.num_bolts_per_side} bolts per side).`,
      );
      return bestSolution.inputs;
    }
    optimizationLog.push(
      `Flange splice optimization failed. Last failure reason: ${lastFailureReason}`,
    );
    return null;
  }
  ...
  function optimizeWebBolts(inputs, optimizationLog) {
    const MAX_TOTAL_BOLTS_PER_SIDE = 48;
    const webBoltDiameters = inputs.optimize_diameter_check
      ? AISC_SPEC.standardBoltDiameters
      : [inputs.D_wp];
    let bestSolution = null;
    let lastFailureReason = "No valid configuration found within limits.";
    ...
    if (bestSolution) {
      ...
      optimizationLog.push(
        `Web splice optimized to ${Nc_wp} column(s) and ${Nr_wp} row(s) of ${D_wp}" bolts (${bestSolution.num_bolts_per_side} bolts per side).`,
      );
      return bestSolution.inputs;
    }
    optimizationLog.push(
      `Web splice optimization failed. Last failure reason: ${lastFailureReason}`,
    );
    return null;
  }
  ...
  function runOptimization(inputs) {
    ...
    const finalOptimizedInputs = optimizeWebBolts(
      optimizedFlangeInputs,
      optimizationLog,
    );
...
const baseBreakdownGenerators = {
  "Flange Bolt Shear": ({ check, details }, common) => {
    const wasReducedText = check.wasReduced
      ? `<br><span class="text-yellow-600">Note: F<sub>nv</sub> was reduced by 20% for long joint length.</span>`
      : "";
  ...
  "Bolt Bearing": ({ check, details, demand }, common) => {
    const tearout_coeff = common.inputs.deformation_is_consideration
      ? 1.2
      : 1.5;
    const bearing_coeff = common.inputs.deformation_is_consideration
      ? 2.4
      : 3.0;
  ...
  "Web Bolt Shear/Tension Interaction": ({ check }, common) =>
    common.format_list([
  ...
  "Beam Section Tensile Rupture": ({ check }, common) =>
    common.format_list([
  ...
  "Flange Bolt Tension with Prying": (data, common) => {
    ...
    const outer_pry = details.outer
      ? `Outer Plate Q = ${common.fmt(details.outer.Q)} kips (t<sub>c</sub>=${common.fmt(details.outer.tc, 3)} in)`
      : "Outer Plate: No prying force";
    const inner_pry = details.inner
      ? `Inner Plate Q = ${common.fmt(details.inner.Q)} kips (t<sub>c</sub>=${common.fmt(details.inner.tc, 3)} in)`
      : "";
    return common.format_list([
  ...
  "Web Bolt Tension with Prying": (data, common) =>
    baseBreakdownGenerators["Flange Bolt Tension with Prying"](data, common),
  ...
  Compression: ({ check, details }, common) => {
    ...
    if (check.slenderness > 25) {
      fcr_calc =
        `<li>Elastic Buckling Stress (F<sub>e</sub>) = (&pi;² &times; E) / &lambda;² = (&pi;² &times; ${common.fmt(check.E)}) / ${common.fmt(check.slenderness)}² = ${common.fmt(check.Fe)} ksi</li>` +
...
function getBreakdownGenerator(name) {
  ...
  if (name.includes("Compression"))
    return baseBreakdownGenerators["Compression"];
  if (name.includes("GSY")) return baseBreakdownGenerators["GSY"];
  if (name.includes("NSF")) return baseBreakdownGenerators["NSF"];
  if (name.includes("Block Shear"))
    return baseBreakdownGenerators["Block Shear"];
  if (name.includes("Bolt Bearing"))
    return baseBreakdownGenerators["Bolt Bearing"];
  if (name.includes("Web Bolt Group Shear (ICR)"))
    return baseBreakdownGenerators["Web Bolt Group Shear (ICR)"];
  if (name.includes("Shear Yield"))
    return baseBreakdownGenerators["Shear Yield"];
  if (name.includes("Web Bolt Slip"))
    return baseBreakdownGenerators["Web Bolt Slip"];
  if (name.includes("Shear Rupture"))
    return baseBreakdownGenerators["Shear Rupture"];
  if (name.includes("Flexural Rupture"))
    return baseBreakdownGenerators["Beam Flexural Rupture"];
  if (name.includes("Flexural Yielding"))
    return baseBreakdownGenerators["Beam Flexural Yielding"];
  if (name.includes("Web Bolt Tension with Prying"))
    return baseBreakdownGenerators["Web Bolt Tension with Prying"];
  if (name.includes("Beam Flange Tensile Rupture"))
    return baseBreakdownGenerators["NSF"];
  if (name.includes("Plate Thickness for Prying"))
    return baseBreakdownGenerators["Plate Thickness for Prying"];
  if (name.includes("Spliced Member Moment Capacity"))
    return baseBreakdownGenerators["Spliced Member Moment Capacity"];
...
function generateSpliceBreakdownHtml(name, data, inputs) {
  ...
  const common = {
    inputs,
    fmt: (val, dec = 2) =>
      val !== undefined && val !== null && !isNaN(parseFloat(val))
        ? parseFloat(val).toFixed(dec)
        : "N/A",
    format_list: (items) =>
      `<ul class="list-disc list-inside space-y-1">${items.map((i) => `<li class="py-1">${i}</li>`).join("")}</ul>`,
    ...
    final_capacity:
      design_method === "LRFD"
        ? (check?.Rn || 0) * (check?.phi ?? 0.75)
        : (check?.Rn || 0) / (check?.omega || 2.0),
  };
...
function renderLoadSummary(rawInputs, final_loads, demands, inputs) {
  ...
  const loadNote = isCapacityDesign
    ? " (Calculated from Member Capacity)"
    : " (User Input)";
  const rows = [
    ...
    { type: "subheader", content: "Load Distribution" },
    {
      cells: [
        ...
        `T<sub>M</sub> = M / (d-t<sub>f</sub>) = (${M_load.toFixed(2)}*12) / ${moment_arm_flange.toFixed(3)}`,
        `${flange_force_from_moment.toFixed(2)} kips`,
      ],
    },
    {
      cells: [
        "&nbsp;&nbsp;&nbsp;Flange Force from Axial",
        `T<sub>P</sub> = P / 2 = ${Axial_load.toFixed(2)} / 2`,
        `${axial_per_flange.toFixed(2)} kips`,
      ],
    },
    {
      cells: [
        "&nbsp;&nbsp;&nbsp;Total Flange Tension",
        "T<sub>u</sub> = T<sub>M</sub> + T<sub>P</sub>",
        `${total_flange_demand_tension.toFixed(2)} kips`,
      ],
    },
    {
      cells: [
        "&nbsp;&nbsp;&nbsp;Total Flange Compression",
        "C<sub>u</sub> = T<sub>M</sub> - T<sub>P</sub>",
        `${total_flange_demand_compression.toFixed(2)} kips`,
      ],
    },
    { type: "subheader", content: "Web Splice Forces" },
    {
      cells: [
        "&nbsp;&nbsp;&nbsp;Shear on Web Splice",
        "V<sub>web</sub> = V",
        `${V_load.toFixed(2)} kips`,
      ],
    },
    {
      cells: [
        "&nbsp;&nbsp;&nbsp;Moment Resisted by Web",
        "M<sub>web</sub> = M<sub>total</sub> - M<sub>flange_splice</sub>",
        `${Mu_resisted_by_web.toFixed(2)} kip-in`,
      ],
    },
    {
      cells: [
        "&nbsp;&nbsp;&nbsp;Horizontal Force on Web",
        "H<sub>w</sub> = M<sub>web</sub> / (0.75 * H<sub>wp</sub>)",
        `${Hw.toFixed(2)} kips`,
      ],
    },
  ].map((row) =>
...
function renderSpliceInputSummary(inputs) {
  const {
    ...
    t_fp_inner,
    L_fp_inner,
    flange_plate_Fy_inner,
    flange_plate_Fu_inner,
    Nc_fp,
    Nr_fp,
    D_fp,
    bolt_grade_fp,
  ...
  const sections = [
    {
      ...
      rows: [
        ...
        { cells: ["Splice Gap", `${gap}"`] },
        {
          cells: [
            "Member",
            `W-Shape (d=${member_d}", b<sub>f</sub>=${member_bf}", t<sub>f</sub>=${member_tf}", t<sub>w</sub>=${member_tw}")`,
          ],
        },
        {
          cells: [
            "Member Material",
            `F<sub>y</sub>=${member_Fy} ksi, F<sub>u</sub>=${member_Fu} ksi`,
          ],
        },
      ],
    },
      ...
      rows: [
        {
          cells: [
            "Outer Plate",
            `PL ${H_fp}" &times; ${L_fp * 2}" &times; ${t_fp}"`,
          ],
        },
        {
          cells: [
            "Outer Plate Material",
            `F<sub>y</sub>=${flange_plate_Fy} ksi, F<sub>u</sub>=${flange_plate_Fu} ksi`,
          ],
        },
        ...(num_flange_plates == 2
          ? [
              {
                cells: [
                  "Inner Plate",
                  `2 x PL ${H_fp_inner}" &times; ${L_fp_inner * 2}" &times; ${t_fp_inner}"`,
                ],
              },
              {
                cells: [
      ...
      rows: [
        {
          cells: [
            "Configuration",
            `${Nc_fp * Nr_fp * 4} total bolts (${2 * Nc_fp} cols &times; ${2 * Nr_fp} rows)`,
          ],
        },
        {
          cells: [
            "Bolt Details",
            `&empty;${D_fp}" ${bolt_grade_fp} (${threads_included_fp ? "Threads Included" : "Threads Excluded"})`,
          ],
        },
      ...
      rows: [
        {
          cells: [
            "Web Plate(s)",
            `${num_web_plates} &times; PL ${H_wp}" &times; ${L_wp * 2}" &times; ${t_wp}"`,
          ],
        },
        {
          cells: [
            "Web Plate Material",
            `F<sub>y</sub>=${web_plate_Fy} ksi, F<sub>u</sub>=${web_plate_Fu} ksi`,
          ],
        },
      ],
    },
      ...
      rows: [
        {
          cells: [
            "Configuration",
            `${Nc_wp * Nr_wp * 2} total bolts (${2 * Nc_wp} cols &times; ${Nr_wp} rows)`,
          ],
        },
        {
          cells: [
            "Bolt Details",
            `&empty;${D_wp}" ${bolt_grade_wp} (${threads_included_wp ? "Threads Included" : "Threads Excluded"})`,
          ],
        },
...
async function populateReportDiagrams() {
  ...
  const engine =
    typeof bjsEngine !== "undefined" ? bjsEngine : window.bjsEngine;
  const scene = typeof bjsScene !== "undefined" ? bjsScene : window.bjsScene;
  if (img3d && engine && scene && scene.activeCamera) {
    try {
      scene.executeWhenReady(() => {
        const oldClearColor = scene.clearColor ? scene.clearColor.clone() : new BABYLON.Color4(0.2, 0.2, 0.3, 1);
      ...
      console.warn("Failed to capture 3D screenshot:", e);
      img3d.alt = "3D Diagram failed";
    }
  }
  ...
  const renderView = (view) => {
    ...
    try {
      ...
      let w = 800, h = 400; // Defaults
      const vb = tempSvg.getAttribute("viewBox");
      if (vb) {
      ...
      img.onload = () => {
        ...
        const scale = 2; // Scale up for better resolution in report
...
function renderResults(results, rawInputs) {
    ...
    cat.checks.forEach((name) => {
      if (checks[name]) {
        ...
        if (name === "Plate Thickness for Prying") {
          ...
          ratio =
            design_capacity > 0 ? Math.abs(demand) / design_capacity : Infinity;
        }
        if (ratio > maxRatio) {
          maxRatio = ratio;
  ...
  const loadSummaryData = renderLoadSummary(
    rawInputs,
    final_loads,
    demands,
    inputs,
    ...
    if (hasChecks) {
      checkList.forEach((name) => {
        if (checks[name]) {
          ...
          if (name === "Plate Thickness for Prying") {
            ...
            ratio =
              display_capacity > 0
                ? display_demand / display_capacity
                : Infinity;
          } else {
            ...
            ratio =
              display_capacity > 0
                ? Math.abs(display_demand) / display_capacity
                : Infinity;
          }
          status =
            ratio <= 1.0
              ? `<span class="pass">${getTranslation("pass")}</span>`
              : `<span class="fail">${getTranslation("fail")}</span>`;
          let demand_unit = "kips",
            capacity_unit = "kips";
          if (name === "Plate Thickness for Prying") {
            demand_unit = "in (req)";
            capacity_unit = "in";
      ...
      report.addTableSection(tableTitle, {
        headers: [
          ...
          getTranslation("demand"),
          getTranslation("capacity"),
          getTranslation("ratio"),
...
<div class="grid grid-cols-3 gap-4 mb-6">
  ...
  inputSummarySections.forEach((section) => {
    report.addTableSection(section.title, {
      headers: [getTranslation("parameter"), getTranslation("value")],
      rows: section.rows,
    });
  });
  ...
  if (geomChecks["Flange Bolts"]) {
    addGeomRow(
      getTranslation("flange_bolt_edge_dist_long"),
      geomChecks["Flange Bolts"].edge_dist_long,
    );
    addGeomRow(
      getTranslation("flange_bolt_edge_dist_tran"),
      geomChecks["Flange Bolts"].edge_dist_tran,
    );
    addGeomRow(
      getTranslation("flange_bolt_edge_dist_gap"),
      geomChecks["Flange Bolts"].edge_dist_gap,
    );
    addGeomRow(
      getTranslation("flange_bolt_spacing_pitch"),
      geomChecks["Flange Bolts"].spacing_col,
    );
    addGeomRow(
      getTranslation("flange_bolt_spacing_gage"),
      geomChecks["Flange Bolts"].spacing_gage,
    );
    addGeomRow(
      getTranslation("flange_bolt_spacing_pitch"),
      geomChecks["Flange Bolts"].max_spacing_col,
      true,
    );
    addGeomRow(
      getTranslation("flange_bolt_spacing_gage"),
      geomChecks["Flange Bolts"].max_spacing_row,
      true,
    );
  }
  if (geomChecks["Web Bolts"]) {
    addGeomRow(
      getTranslation("web_bolt_edge_dist_long"),
      geomChecks["Web Bolts"].edge_dist_long,
    );
    addGeomRow(
      getTranslation("web_bolt_edge_dist_tran"),
      geomChecks["Web Bolts"].edge_dist_tran,
    );
    addGeomRow(
      getTranslation("web_bolt_edge_dist_gap"),
      geomChecks["Web Bolts"].edge_dist_gap,
    );
    addGeomRow(
      getTranslation("web_bolt_spacing_pitch"),
      geomChecks["Web Bolts"].spacing_col,
    );
    addGeomRow(
      getTranslation("web_bolt_spacing_gage"),
      geomChecks["Web Bolts"].spacing_row,
    );
    addGeomRow(
      getTranslation("web_bolt_spacing_pitch"),
      geomChecks["Web Bolts"].max_spacing_col,
      true,
    );
    addGeomRow(
      getTranslation("web_bolt_spacing_gage"),
      geomChecks["Web Bolts"].max_spacing_row,
      true,
    );
  }
  report.addTableSection(getTranslation("geometry_spacing_checks_title"), {
    headers: [
      getTranslation("check_column_header"),
      getTranslation("actual_in"),
      getTranslation("min_required_in"),
      getTranslation("status"),
    ],
    rows: geomRows,
  });
...
const inputIds = [
  ...
  "member_Sx",
  "member_Sx",
  "develop_capacity_check",
  "deformation_is_consideration",
  "g_gage_fp",
  "optimize_bolts_check",
  "optimize_diameter_check",
  "optimize_web_plates_check",
  "optimize_flange_plates_check",
  "num_flange_plates",
  "flange_plate_material",
  "flange_plate_Fy",
  "flange_plate_Fu",
  "H_fp",
  "t_fp",
  "L_fp",
  "flange_plate_material_inner",
  "flange_plate_Fy_inner",
  "flange_plate_Fu_inner",
  "H_fp_inner",
  "t_fp_inner",
...
const handleRunSpliceCheck = createCalculationHandler({
  ...
  validatorFunction: (inputs) => {
    const { errors, warnings } = validateInputs(
      Object.keys(inputs),
      validationRules.splice,
    );
    const clearWebDepth = inputs.member_d - 2 * inputs.member_tf;
    if (inputs.H_wp > clearWebDepth) {
      errors.push(
        `Web plate height (${inputs.H_wp}") cannot exceed the beam's clear web depth (${clearWebDepth.toFixed(3)}").`,
      );
    }
...
initializeApp({
  ...
  onReady: () => {
    ...
    const shapeTypeSelect = document.getElementById("member_shape_type");
    if (shapeTypeSelect)
      shapeTypeSelect.addEventListener("change", populateShapeDropdown);
    populateShapeDropdown();
    document
      .getElementById("aisc_shape_select")
      .addEventListener("change", handleShapeSelection);
    ...
    const toggleDimensionsBtn = document.getElementById(
    ...
    if (toggleDimensionsBtn) {
      toggleDimensionsBtn.addEventListener("click", () => {
        if (dimensionElements) {
          ...
          dimensionElements.meshes.forEach((mesh) => {
            if (mesh) mesh.isVisible = areDimensionsVisible;
          });
          dimensionElements.labels.forEach((label) => {
            if (label) label.isVisible = areDimensionsVisible;
          });
          toggleDimensionsBtn.textContent = areDimensionsVisible
            ? "Hide Dimensions"
            : "Show Dimensions";
        }
      });
